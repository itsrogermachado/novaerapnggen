...
# 1. Imports
app = app.replace('import { Format, TextItem, LibraryItem, Foreground, LogoState, CanvasState, ElementLayouts } from "@/types/canvas";', 
                  'import { Format, LibraryItem, Foreground, LogoState, CanvasState, ElementLayouts } from "@/types/canvas";')

# 2. State definition
old_state = '''  const [texts, setTexts] = useState<TextItem[]>([
    {
...
app = app.replace(old_state, '')

# 3. useCanvasHistory hook
old_history = '''  const { past, future, setPast, setFuture, saveToHistory, undo, redo, goToHistoryState } = useCanvasHistory({
...
new_history = '''  const { past, future, setPast, setFuture, saveToHistory, undo, redo, goToHistoryState } = useCanvasHistory({
...
new_literal = '''      bgImg,
      ...
      logo,
      format,'''
app = app.replace(old_literal, new_literal)

# 7. generateAndCheck texts
app = app.replace('let candidateTexts = [...texts];\n', '')
old_generate = '''      const layout = generateCohesiveLayout(
        format,
        foregrounds.length,
        texts.length,
...
new_generate = '''      const layout = generateCohesiveLayout(
        format,
        foregrounds.length,
        true, // force logo existence
        Math.floor(Math.random() * 3),
      );'''
app = app.replace(old_generate, new_generate)

# 8. randomize texts mapping
old_randomize = '''      const randomFont = FONTS[Math.floor(Math.random() * FONTS.length)].id;
...
app = app.replace(old_funcs, '')

# 10. onPointerDown
old_pd = '''    e: React.PointerEvent,
...
# 13. UI - Texts Property Panel
old_texts_panel = '''              {texts.find((t) => t.id === selectedId) && (
...
app = app.replace(old_texts_panel, '')

# 14. UI - Texts Menu
old_texts_menu = '''          {/* Texts */}
          <div className="space-y-3">
...
app = app.replace(old_texts_menu, '')

# 15. UI - Texts Canvas mapping
old_texts_canvas = '''            {/* Draggable Texts with observer scaled font size */}
            {texts.map((t) => {
              const selectedFont = FONTS.find((f) => f.id === (t.font || "inter")) || FONTS[0];
...
app = app.replace(old_texts_canvas, '')

# 16. Timeline visualizer
